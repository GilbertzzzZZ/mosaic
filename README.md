...
Declarative content blocks for [Obsidian](https://obsidian.md) notes: write a declaration in md / mdx, get rich interactive content — no changes to the source file, no external site required. Notes compose like a mosaic, one block at a time: charts are available today; metric grid, card and other block types are planned. See [docs/mosaic-intro.md](docs/mosaic-intro.md) ([中文](docs/mosaic-intro-zh.md)).

Charts are the first block type: rendered with [Ant Design Charts](https://charts.ant.design/) from inline data, vault CSV files, Dataview queries, or external dataset manifests (`.dataset.json`) with time-range filtering and granularity rollup (see [docs/dataset-guide.md](docs/dataset-guide.md)).
...
Use command `Mosaic: Insert Template` -> `Dataviewjs Example (Column)` to insert code block.

![image](https://user-images.githubusercontent.com/150803/140684190-fa6a08ea-3394-44fe-ae92-265810f6b9a9.png)

## Chart Wizard
Use command `Mosaic: Wizard` to insert code block.
...
Use command `Mosaic: Import data from external CSV file` to insert data from CSV file.
...
- Copy over `main.js`, `styles.css`, `manifest.json` to your vault `VaultFolder/.obsidian/plugins/mosaic/`.
