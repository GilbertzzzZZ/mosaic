...
> Chart 是 Mosaic 唯一走图表库出图的区块，也是唯一有三种写法的区块。本文解释写法分裂、模式边界、类型体系、标签可读性与格式化体系背后的设计理由。用法与属性表见 [../chart.md](../guides/chart.md)。
...
- **`combo`（单刻度）**：柱与线共享同一把尺子——两轴钉住相同的下界零与相同的上界。适合「同一单位、想同图比大小」的场景；图例顺序跟随属性书写顺序，作者写 `lines` 在前则线系列排前。
...
配套地，Y 轴上限自动加 8% 头部空间，避免最大值点贴顶、标签被上边界挤压。

## 单位格式化体系

原则：**格式化只做展示层加工，不改数据**。所有数值统一千分位分组、最多两位小数。单位的呈现位置按单位性质分三档：

- 百分号作后缀直接跟在数值上——「42%」比「42（轴标题：%）」的读图成本低得多；
- 货币经词表归一成符号作前缀（中文与英文的多种写法都归一到 ¥ 或 $）——货币习惯读作前缀；
- 其余单位只画在轴标题上——把「件」「人」逐个缀在每个标签后只会增加视觉噪音。
...
- [../chart.md](../guides/chart.md) / [../dataset-guide.md](../guides/dataset-guide.md)——用法、属性表与排错清单
