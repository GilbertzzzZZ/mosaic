...
实测数据把这一点摆得很清楚：扫过 真实笔记库 全部 49 张内联表，只有 4 张越过阈值，而这 4 张还全部来自同一份微信日数据。也就是说 92% 的表是一个样子，8% 是另一个样子，那 8% 的成因与内容语义毫无关系。
...
表格框体有一个高度上限（约 26 行），超出部分在框内滚动，滚动时表头与首列钉住。这三件事是**一件事**，顺序不能颠倒：`position: sticky` 需要一个高度受限的滚动祖先才有参照物，没有 `max-height`，`top: 0` 永远不会触发。

早期版本里「表头吸顶」是复杂度启发式派生出的一个开关，代码写得对，但滚动容器没有高度上限——表格是整体撑开的，页面滚动时整张卡片一起走，于是那条规则从上线到删除**一次都没有真正生效过**。这是一个典型的「配置写对了、能力没生效」：单元测试断言 class 挂上了，而 class 挂上不等于 sticky 会触发。

现在三件事一起做，并且**无条件**：所有 DataTable 一视同仁，没有阈值、没有属性、没有 class 开关。大表滚起来钉住、小表根本不滚，是同一条规则在不同内容量下的自然结果，不是两种不同的组件。

**一处必须配套的改动**：表格的 `border-collapse` 从 `collapse` 改成 `separate` + `border-spacing: 0`，所有分隔线改用 inset box-shadow。collapse 模式下边框的绘制归表格而不归单元格，sticky 的表头和首列滚走时，边框会留在原地——线和格子分家。separate 让每个单元格自己画边，位置跟着单元格走。inset（画在单元格内侧）而不是外投影，否则会被相邻单元格的背景盖掉。
