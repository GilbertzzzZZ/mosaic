...
**Planned types** (superset of 早期内部实现)

- Mosaic aims to cover every component type 早期内部实现 supports, so the same documents render fully in both places: `Chart` (done), `DataTable`, `MetricGrid`, `Timeline`, `DecisionBox`, `FlowDiagram`.
...
- (Placeholder) MetricGrid, Timeline, DecisionBox and FlowDiagram block types — completing the 早期内部实现 superset.
