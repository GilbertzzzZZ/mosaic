...
- Mosaic 目标覆盖 git-leaf 支持的全部组件类型，让同一份文档在两边都能完整渲染：`Chart`（已完成）、`DataTable`、`MetricGrid`、`Timeline`、`DecisionBox`、`FlowDiagram`。
...
- （占位）MetricGrid、Timeline、DecisionBox、FlowDiagram 内容块类型——补齐 git-leaf 超集。
