# 早期内部实现 五类 mdx-lite 组件渲染规格调研

调研目标仓库：`$HOME/projects/早期内部实现`（只读）
...
核心源文件：
- `$HOME/projects/早期内部实现/src/content/mdx-lite.mjs`（渲染函数主体，1411 行）
- `$HOME/projects/早期内部实现/src/content/table-complexity.mjs`（DataTable 工具栏与复杂度判定）
- `$HOME/projects/早期内部实现/src/content/table-layout.mjs`（DataTable 列宽自动布局算法）
- `$HOME/projects/早期内部实现/public/styles.css`（组件 CSS，第 4400-5480 行为 mdx 相关样式）
- `$HOME/projects/早期内部实现/public/app.js`（第 9417-9443 行：DataTable 交互 enhanceTables）
- `$HOME/projects/早期内部实现/public/dataset-view.js`（外部 dataset 的 granularity 切换与 meta 渲染）
- `$HOME/projects/早期内部实现/docs/mdx-lite-guide.md`（权威语法文档）
- `$HOME/projects/早期内部实现/docs/mdx-lite-components-demo.mdx`（可视化回归 fixture，含真实样例数据）
...
代码里 `is-${status}` class 确实存在（`accepted`/`proposed`/`rejected`/`superseded`/`default`），但检索 CSS 全文（`grep mdx-decision-box`）发现**只有 `.mdx-decision-box` 一条基础规则，没有 `.mdx-decision-box.is-accepted` 等针对不同 status 的差异化样式**——也就是说当前 早期内部实现 实现里这些 class 目前是"预留但未接颜色"的状态，视觉上四种 status 长得完全一样，只有徽标文字不同。**Mosaic 复刻时这是一个可以按需增强的点**（比如给 accepted 加绿色左边框、rejected 加红色），但不属于"忠实复刻现状"的必需项；如果目标是逐像素对齐，应保持无差异。
...
**Mosaic 实现建议**：这套算法本质是标准的"分层 DAG 布局 + 层内居中 + 三次贝塞尔连接"，**不依赖任何外部库**，用纯 JS/TS 在 Mosaic 里原样移植是完全可行的（大约 150 行代码，上面已给出全部常量与公式）。不建议引入 D3/dagre 等重量级依赖去"更好地"布局——那样反而会产生与 早期内部实现 视觉不一致的结果（尤其是同层节点顺序、边的弯曲方式）。如果 Mosaic 需要处理带环的图，注意第 5 步的"环节点各自单独成层"这个退化规则也要复刻，否则视觉差异会很明显。
