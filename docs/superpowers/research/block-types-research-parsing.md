# git-leaf MDX-lite 五类组件解析契约研究

> 证据来源：`git-leaf` 仓库（只读研究，未修改）。核心文件：
...
本报告只覆盖 Mosaic 需要的五类：**DataTable、MetricGrid、Timeline、DecisionBox、FlowDiagram**。git-leaf 还有第六类 `Chart`，其渲染逻辑不在范围内，但它与 DataTable 共享全部 payload 提取机制和 dataset 查询机制，因此在"共享机制"与"dataset 查询"两节会引用 Chart 分支作对照证据。
...
对 Mosaic 的启示：解析（识别标签边界 + 提取 attributes/content）与渲染（各组件的 HTML/DOM 生成）在 git-leaf 中是两个独立阶段，且解析阶段与 markdown 引擎耦合较浅（`mdxLiteComponentOpeningAtLines`/`mdxLiteComponentBlockAtLines`/`parseMdxLiteAttributes` 全是不依赖 markdown-it 状态的纯函数，只需要一个 `lines: string[]` 数组）。这两个探测函数是 Mosaic 应该逐字节移植的核心。
...
git-leaf 是"本地 Web 服务 + 浏览器前端"架构，dataset 模式因此被拆成两个阶段：
...
### 建议逐字节移植（纯函数、零/极浅外部依赖，逻辑必须与 git-leaf 位级一致才能保证同一份 `.md` 在两边渲染结果一致）
...
| `queryDataset` 及其内部所有辅助函数（dataset 模式需要时） | `dataset-query.mjs` | 整个文件 620 行都是纯函数（无 fs/网络依赖），是 DataTable dataset 查询语义的唯一真源；已在 git-leaf 中作为独立模块存在，符合任务描述"像 dataset-query 已经被移植过"的先例 |

### 建议重新实现（依赖 Node.js `fs`/HTTP、或与 Obsidian vault API 强相关，逻辑需要但载体要换）

| 关注点 | git-leaf 实现 | Mosaic 应做的调整 |
|---|---|---|
...
| 错误呈现（`componentError` + i18n `component.error` 模板） | `mdx-lite.mjs` + `public/i18n.js` | 错误消息原文（各处 `throw new Error(...)` 的字符串）建议保留一致，方便用户从 git-leaf 文档迁移过来时行为可预期；外层包裹样式/i18n 机制按 Obsidian 插件自身规范重写 |
