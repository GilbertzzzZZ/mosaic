...
- **开标签必须单行**：只有「完整的开标签独占一行」才会触发 markdown 的 HTML block 规则，把标签体连同 CSV 围栏整体交给插件；开标签换行会被当作普通段落，后面的围栏把内容切碎，无法接管（自闭合标签没有这个限制，因为标签体内没有围栏）。属性多到想换行时，改用代码块写法——frontmatter 天然一行一个属性。
