// Obsidian 打开文件时会在 section 元素尚未挂载（宽度 0），或虽已挂载但宽度
// 还是一个过渡值时就调用处理器；此时出图会让 AntV 在错误的画布几何上做标签
// 防碰撞，大量标签被误藏，且被摘离缓存的 section 会把坏画布一直保留。等宿主
// attach 且有宽度再渲染；过渡宽度由 ChartFigure 的宽度监听重建兜底。
